...
let
    ...
    function cube!(ax; a1=1, a2=1, timeLonLat=(40, 50, 20), cxyz=(0, 0, 0),
        ...
        bsurf = heatmap!(ax, y .+ cxyz[2], z .+ cxyz[3], backF, transformation=(:yz, -a2 + cxyz[1]),
            colormap=cmap, transparency=transparent, colorrange=(lowLimit, highLimit)) ## back
    ...
    function cubePlot()
        ...
        ax = Axis3(fig[1, 1], aspect=:data, perspectiveness=0.5, xlabel="time",
        ...
        Colorbar(fig[1, 2], cbox, label="temperature", height=Relative(0.5))
