...
#keysTime = unique(sort(earthquakes[!, "time"]))
...
pltobj = meshscatter!(ax, @lift(toPoints3D[1:$index]); markersize = @lift(ms[1:$index] / 20 .+ 0.001),
    color = 1:length(toPoints3D), # @lift(earthquakes.mag[1:$index]),
    colormap = tuple.(to_colormap(:Spectral_11)[5:end-5], 0.75),
    shading = true,
    ambient = Vec3f(1, 1, 1) #colorrange = (2.5, 8.5)
)
surface!(ax, sphere(; r = 1.0)..., color = tuple.(earth_img, 0.1), shading = true,
    transparency = true)
cbar = Colorbar(fig[1, 2], pltobj, #label = "Date",
...
rotate!(ax.scene, 3.0)
fig
save("earthquakesTimeSd_1.png", fig, px_per_unit = 2.0)
...
save("earthquakesTimeSd_2.png", fig, px_per_unit = 2.0)
