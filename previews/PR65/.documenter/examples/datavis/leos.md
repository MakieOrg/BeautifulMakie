...
```ansi
WARNING: Constructor for type "BezierPath" was extended in `Main` without explicit qualification or import.
  NOTE: Assumed "BezierPath" refers to `GLMakie.BezierPath`. This behavior is deprecated and may differ in future versions.`
  NOTE: This behavior may have differed in Julia versions prior to 1.12.
  Hint: If you intended to create a new generic function of the same name, use `function BezierPath end`.
  Hint: To silence the warning, qualify `BezierPath` as `GLMakie.BezierPath` in the method signature or explicitly `import GLMakie: BezierPath`.
```
